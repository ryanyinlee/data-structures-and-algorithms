...
function transformToLis(obj){
  ...
  for (const property in obj) {
    returnArray.push(`<li>${property}: ${obj.name}</li>`);
    returnArray.push(`<li>${property}: ${obj.age}</li>`);
  }
  returnArray.splice(1,1);
  returnArray.splice(1,1);
  //console.log(returnArray);
  return returnArray;
}; // Okay I know this probably wasn't how it was supposed to be solved, but it works ;v
...
return count;
}; // nested for loops are bad, but I am not sure how to solve it the other way.
...
const totalSum = (input) => {
  ...
  for(let i = 0; i <= input.length - 1; i++) {
          let innerSum = input[i].reduce(reducer);
