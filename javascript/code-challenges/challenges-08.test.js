...
// get length of array within object
// sort by length of array
// if array length is the same, sort by name

const sortByChildren = (charArray) =>  charArray.sort((firstEl, secondEl) => firstEl.children.length - secondEl.children.length);
...
const containsW = (str) => {
  const regex = /w/;
  let testText = str;
  let returnData = regex.test(testText);
  return returnData;
}
...
const isNum = (input) => {
  ...
  let returnData = regex.test(testString);
  return returnData;
};
...
const isCapitalized = (str) => {
  ...
  let returnData = str.match(regex);

  //let returnData2 = returnData.match(regex2);
...
const citiesAtoJ = (arr) => {
  ...
  for (let i = 0; i < arr.length; i+=1) {
      let pushValue = arr[i].match(regex)
      
      if ((pushValue != null) && (pushValue == arr[i])) {
  ...
  // I get back [["Cleveland"], ["Birmingham"], ["Austin"], ["Boston"], ["Hoboken"]]? 
  // nevermind
};
