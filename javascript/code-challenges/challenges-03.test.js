...
const typeNum = (arr) => {
  let numbersOnly = arr.filter( num => !isNaN(num) );
...
const containsAnd = (arr) => {
  let andOnly = arr.filter( param => param.includes('and') );
...
const oddValues = (arr) => {
  let oddsOnly = arr.filter( num => (num % 2) );
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  let notTheSameArray = arr.filter(value => !forbiddenValues.includes(value));

  return notTheSameArray;
};
