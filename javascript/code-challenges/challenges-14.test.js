...
const screenForNames = (arr) => {
  let regex2 = /^\w[abc]Mr. | Mrs. | Ms. | Dr. /i;
  let regex4 = /^mrs?\.\s+[a-z]+\s+[a-z]+$/i;
  let regex5 = /^(?!([MDS]r|Ms)\.).*$/;
  let regex6 = /^(?!Mr.|Ms.|Dr.|Mrs.)\.$/;
...
const toTitleCase = (arr) => {
  ...
  for (let i = 0; i < arr.length; i+=1) {
      ...
      let returnChar = arr[i].charAt(0).toUpperCase();
      let tempString = arr[i].replace(arr[i].charAt(0), returnChar);
...
// and https://stackoverflow.com/questions/1069666/sorting-object-property-by-values
// Markus R
