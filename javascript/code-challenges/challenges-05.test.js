...
const toLastNames = people => {
  let returnArray = [];  
  returnArray = people.map( (prior) => (prior.firstName + ' ' + prior.lastName));
  return returnArray;
};
...
const addValues = (arr) => {
  let returnValue = 0;  
  returnValue = arr.reduce( (prior, next) => (prior + next), 0);
  return returnValue;
};
...
const addPurchases = (arr) => {
  let returnValue =0;  
  const reducer = (prior, next) => (prior + next.purchasePrice)
  returnValue = arr.reduce(reducer, 0);
  return returnValue;
};
...
const countNumberOfElements = (arr) => {
  let returnValue = 0;  
  const reducer = (prior, next) => (prior + next/next)
  returnValue = arr.reduce(reducer, 0);
  return returnValue;
};
...
const returnNames = (arr) => {
  ...
  let afterSplit = [];     
  const reducer = (firstObj, nextObj) => (afterSplit.push(nextObj.name))
  arr.reduce(reducer, 0);
  // values needed: arr[0].name, arr[1].name, arr[2].name, arr[3].name ,arr[4].name
  // push to an array
  return afterSplit;
};
...
const reversedString = (str) => {
  ...
  for (let i = str.length; i >= 0; i-=1 ) {
      pushArray.push(str.charAt(i));
  }

  const reducer = (firstObj, nextObj) => (firstObj + nextObj);
  returnString = pushArray.reduce(reducer);
  return returnString;
};

// original string: "Code 103"
// want: "103 edoC"
// plan: take the string a part using str.charAt()
// need to start at str.charAt(str.length);
// reduce it back together
