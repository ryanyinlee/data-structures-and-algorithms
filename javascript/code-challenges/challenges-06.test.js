...
const getNames = (arr) => {
  let returnArray = [];
  let holdArray = [];
  holdArray = arr.map((prior) => prior.name);
  returnArray = holdArray.map(prior => prior.split("").reverse().join(""));
  return returnArray;
};
// Googled this one: https://stackoverflow.com/questions/958908/how-do-you-reverse-a-string-in-place-in-javascript
// Split rips apart the string into an array.
// Reverse reverses the array.
// Join takes the now reversed array and squishes it back together.
...
// this is the first time I wrote something that passed the test the first time :v
