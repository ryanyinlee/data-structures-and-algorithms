...
const addExclamation = (array) => {
  const localArray = [];
  function addBang(string) {
    return string + '!';
}

array.forEach((string) => localArray.push(addBang(string)));
...
const allUpperCase = (arr) => {
  ...
  arr.forEach((string) => localArray.push(upperCase(string)));
...
const addNumbers = (num, array1, times, callback) => {
...
------------------------------------------------------------------------------------------------ */
// With help from Spencer Tower and Erik Savage
const createList = (availableItems) => {
  //const test = available => (available === true);
  const newArray =[];
  availableItems.forEach(fruitObject => {  if (fruitObject.available === true ) {newArray.push(fruitObject.name) } })
;
