...
const firstLetters = (arr) => {
  ...
  let merged = [];
  // for (let i = 0; i < arr.length; i += 1){
  //   pushArray.push(arr[i].[0]);
  // }
  // return pushArray;

  for (let i = 0; i < arr.length; i +=1){
    var firstLetter = arr[i].match(/\b(\w)/g); // get a, then, b, then, c all in different arrays
    // console.log(firstLetter)
    pushArray.push(firstLetter);
    // console.log(pushArray);
  }
  for (let i = 0; i < arr.length; i +=1){
    merged = [].concat.apply([], pushArray);
  }
...
------------------------------------------------------------------------------------------------ */
// https://regex-generator.olafneumann.org/?sampleText=%3A)&flags=i&onlyPatterns=false&matchWholeLine=false&selection=
...
const standardizePhoneNumbers = (arr) => {
  ...
  let merged =[];
  let pushArray = [];
  let numsOnlyDotComTheDatingSiteForNumsOnly = [];

  for (let i = 0; i < arr.length; i +=1){
    numsHere = arr[i].match(regex);
    merged = numsHere.join('');
    pushArray.push(merged);
